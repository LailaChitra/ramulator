...
namespace ramulator
{
    ...
    public:
        ...
        deque<Request> pending; // read requests that are about to receive data from DRAM
                                // chitra code redundant request
        std::array<std::deque<Request>, 8> red;
        ...
        {
            req.arrive = clk;
            if (req.type == Request::Type::READ && find_if(writeq.q.begin(), writeq.q.end(), // COHERENCE
                                                           [req](Request &wreq)
            ...
            auto &selectedDeque = red[req.addr_vec[2]];

            if (req.type == Request::Type::READ)
            {

                auto rit = std::find_if(selectedDeque.rbegin(), selectedDeque.rend(), [req](const Request &r)
                                        { return r.addr == req.addr; });

                if (rit != selectedDeque.rend())
                {
                    // Insert the request after the last occurrence
                    selectedDeque.insert(rit.base(), req);
                    return true;
                }
            }
            ...
            if (req.type == Request::Type::READ)
            {
                auto it = std::find_if(readq.q.begin(), readq.q.end(),
                                       [req](const Request &r)
                                       { return r.addr == req.addr; });

                if (it != readq.q.end())
                {
                    selectedDeque.push_back(req);
                    return true;
                }
            }
            ...
            if (req.type == Request::Type::READ)
            {
                auto it = std::find_if(actq.q.begin(), actq.q.end(),
                                       [req](const Request &r)
                                       { return r.addr == req.addr; });
        ...
        {   
            ...
            if (pending.size())
            {
                ...
                if (req.depart <= clk)
                {
                    ...
                    read_complete1 = true;
                    req.callback(req); // auto read_complete = [&latencies](Request& r){latencies[r.depart - r.arrive]++;}; read_complete=callback
                    rem_req = pending[0];
                    pending.pop_front();
                }


                auto &selectedDeque1 = red[rem_req.addr_vec[2]];
                if (!selectedDeque1.empty())
                {
                    auto it = selectedDeque1.begin();
                    coreid = rem_req.coreid; // how to handle multi core

                    while (it != selectedDeque1.end() && !selectedDeque1.empty())
                    {
                        if (it->addr == rem_req.addr)
                        {
                            ++read_row_hits[coreid];
                            ++row_hits;
                            it->depart = rem_req.depart;
                            it->callback(*it);
                            it = selectedDeque1.erase(it);
                        }
                        else
                        {
                            it++;
